...
const STATUS_CYCLE = [null,"free","busy","maybe"]
const STATUS_CFG = {
  free:  { bg:"#bbf7d0", text:"#14532d", border:"#4ade80", sym:"✓" },
  busy:  { bg:"#fecaca", text:"#7f1d1d", border:"#f87171", sym:"✕" },
  maybe: { bg:"#fef9c3", text:"#713f12", border:"#facc15", sym:"~" },
  null:  { bg:"#f3f4f6", text:"#d1d5db", border:"#e5e7eb", sym:"" },
}
...
export default function App() {
  ...
  // Poll every 15s so others see updates
  useEffect(()=>{
    const t = setInterval(()=>{
  ...
  async function cycleStatus(member, date){
    ...
    setData(prev=>({
      ...prev,
      [member]:{ ...prev[member], overrides:{ ...(prev[member]?.overrides||{}), [dk]:next } }
    }))
    setSaving(true)
    await fetch('/api/availability',{ method:'POST', headers:{'Content-Type':'application/json'},
      body:JSON.stringify({member, date:dk, status:next}) })
    setSaving(false)
    showToast("Saved ✓")
  }
  ...
  function getBestDays(){
    const s={}; DAYS.forEach(d=>{ s[d]={free:0,busy:0,maybe:0,unknown:0} })
    weeks.slice(0,8).forEach(ws=>{
      DAYS.forEach((day,di)=>{
        const date=addDays(ws,di); if(date<TODAY||date>END) return
        MEMBERS.forEach(m=>{
          const st=getStatus(m,date)
          if(st==="free") s[day].free++
          else if(st==="busy") s[day].busy++
          else if(st==="maybe") s[day].maybe++
          else s[day].unknown++
        })
      })
    })
  ...
  if(loading) return <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",fontSize:16,color:"#6b7280"}}>Loading...</div>
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:"#f4f4f8",minHeight:"100vh"}}>
      ...
      <div style={{background:"#18181b",color:"white",padding:"14px 18px",position:"sticky",top:0,zIndex:200,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        <div>
          ...
          <div style={{fontSize:11,color:"#a1a1aa",marginTop:2}}>{saving?"Saving...":"Tap your name, then tap cells to update"}</div>
        </div>
      ...
      <div style={{background:"white",borderBottom:"1px solid #e4e4e7",padding:"10px 16px",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
        <span style={{fontSize:11,color:"#71717a",fontWeight:700}}>You are:</span>
        {MEMBERS.map((m,i)=>(
        ...
        {selMember && <span style={{fontSize:11,color:"#9ca3af",marginLeft:4}}>Tap cell → ? › Free › Busy › Maybe</span>}
      </div>
            ...
            <div key={mi} style={{marginBottom:24}}>
              ...
              <div style={{overflowX:"auto"}}>
                <table style={{borderCollapse:"separate",borderSpacing:2}}>
                  <thead>
                    <tr>
                      <th style={{fontSize:11,color:"#9ca3af",fontWeight:600,textAlign:"left",minWidth:72,padding:"2px 6px"}}>Member</th>
                      {month.weeks.map((ws,wi)=>DAYS.map((day,di)=>{
                        ...
                        return <th key={`${wi}-${di}`} style={{minWidth:30,width:30,fontSize:9,color:"#9ca3af",fontWeight:700,textAlign:"center",padding:"2px 1px"}}>
                          <div>{day[0]}</div>
                    ...
                    {MEMBERS.map((member,mi2)=>(
                      <tr key={member}>
                        ...
                        {month.weeks.map((ws,wi)=>DAYS.map((day,di)=>{
                          ...
                          return <td key={`${wi}-${di}`}>
                            <div onClick={()=>isEd&&cycleStatus(member,date)} style={{
                              width:30,minWidth:30,height:27,lineHeight:"27px",borderRadius:4,textAlign:"center",
                              fontSize:10,fontWeight:800,userSelect:"none",
                              background:cfg.bg,color:cfg.text,border:`1px solid ${cfg.border}`,
                              outline:isEd?`2px solid ${MEMBER_COLORS[mi2]}`:"none",
                              cursor:isEd?"pointer":"default",transition:"transform 0.1s",
                            }}
                            onMouseEnter={e=>{ if(isEd) e.target.style.transform="scale(1.15)" }}
      ...
      {view==="summary" && (
        <div style={{padding:16}}>
          ...
          <div style={{fontSize:12,color:"#6b7280",marginTop:2,marginBottom:14}}>Ranked by confirmed free slots</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:12}}>
            {sortedDays.map((day,rank)=>{
              const s=bestDays[day]||{}
              const total=(s.free||0)+(s.busy||0)+(s.maybe||0)+(s.unknown||0)
              const pct=total?Math.round((s.free||0)/total*100):0
              const isTop=rank===0
              return <div key={day} style={{background:isTop?"#eef2ff":"white",border:`2px solid ${isTop?"#6366f1":"#e4e4e7"}`,borderRadius:12,padding:14,position:"relative"}}>
                ...
                {[["✅ Free","free","#bbf7d0","#14532d"],["❌ Busy","busy","#fecaca","#7f1d1d"],["🤷 Maybe","maybe","#fef9c3","#713f12"],["❓ Unknown",null,"#f3f4f6","#6b7280"]].map(([label,key,bg,col])=>(
                  <div key={label} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                    <span style={{fontSize:11,color:"#6b7280"}}>{label}</span>
                    <span style={{fontSize:11,fontWeight:700,background:bg,color:col,padding:"1px 9px",borderRadius:10}}>{key?s[key]||0:s.unknown||0}</span>
                  </div>
                ))}
                <div style={{marginTop:8,fontSize:11,fontWeight:700,color:isTop?"#6366f1":"#9ca3af"}}>{pct}% free</div>
              </div>
            })}
          </div>
        </div>
      ...
      {toast && <div style={{position:"fixed",bottom:24,right:20,background:"#18181b",color:"white",padding:"10px 18px",borderRadius:10,zIndex:9999,fontSize:13,fontWeight:700,boxShadow:"0 4px 20px rgba(0,0,0,0.3)"}}>{toast}</div>}
    </div>
  )
}
